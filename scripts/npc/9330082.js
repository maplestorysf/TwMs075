...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        sel = selection;
        itemid = select == 1 ? 3993002 : 3993002;
        pricemultipy = select == 1 ? 10000 : 10000;
		if(select >= 3){
			cm.sendYesNo("請問你確定是否要兌換#v"+items[select - 3][sel][0]+"##t"+items[select - 3][sel][0]+"#\r\n\r\n需要#r"+items[select - 3][sel][1]+"#kBS點數");
		} else {
			cm.sendYesNo("請問你確定是否要兌換點數?");
			cm.dispose()
		}
    } else if (status == 3) {
