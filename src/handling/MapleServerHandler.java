...
//        }
    ...
    public void messageReceived(final IoSession session, final Object message) {
        ...
        if (ServerConstants.DEBUG_MODE) {
            String op = RecvPacketOpcode.nameOf(header_num);
            String pHeaderStr = Integer.toHexString(header_num).toUpperCase();
            pHeaderStr = "0x" + StringUtil.getLeftPaddedStr(pHeaderStr, '0', 4);
            final StringBuilder sb = new StringBuilder("[æ¥æ¶] " + op + "(" + pHeaderStr + ") " + slea.available() + "å­å:\n");
        ...
        for (final RecvPacketOpcode recv : RecvPacketOpcode.values()) {
            if (recv.getValue() == header_num) {
                ...
                try {
                    ...
                    if (c.getPlayer() != null) {
                        // 假斷線處理
                        if (World.Find.findChannel(c.getPlayer().getName()) == -1) {
                            switch (recv) {
    ...
    public static final void handlePacket(final RecvPacketOpcode header, final LittleEndianAccessor slea, final MapleClient c, final boolean cs) throws Exception {
        switch (header) {
            ...
            case SET_GENDER:
                ...
                break;
            case LOGIN_PASSWORD:
                CharLoginHandler.handleLoginAccount(slea, c);
                break;
            ...
            case AUTH_SECOND_PASSWORD:
                ...
                break;
            case CLIENT_ERROR:
                FileoutputUtil.log("é¯èª¤æ¥èª.rtf", "Packet: " + HexTool.toStringFromAscii(slea.read((int) slea.available())));
                break;
            // END OF LOGIN SERVER
            case CHANGE_CHANNEL:
            ...
            case CHAR_INFO_REQUEST:
                ...
                break;
            case CHAR_INFO_SET:
                PlayerHandler.CharInfoSetRequest(slea, c);
                break;
            ...
            case VICIOUS_HAMMER:
                ...
                slea.skip(4); // all 0
                c.sendPacket(MTSCSPacket.ViciousHammer(false, 0));
                break;
