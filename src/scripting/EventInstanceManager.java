...
public class EventInstanceManager {
    ...
    public void playerDisconnected(final MapleCharacter chr, int idz) {
        ...
        try {
            ...
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "Event name : " + em.getName() + ", Instance name : " + name + ", method Name : playerDisconnected:\n" + ex);
        } finally {
            wL.unlock();
        }
    ...
    public final void broadcastPlayerMsg(final int type, final String msg) {
        ...
        for (MapleCharacter chr : getPlayers()) {
            chr.getClient().sendPacket(CWvsContext.serverNotice(type, msg));
        }
    }
    ...
    public final void schedule(final String methodName, final long delay) {
        if (ChannelServer.getInstance(1).isShutdown()) {
            System.err.println("schedule - 關閉伺服器中無法調用");
            return;
        }
        if (disposed) {
            return;
