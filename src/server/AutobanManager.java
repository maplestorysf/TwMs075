        ...
        try {
            ...
            if (this.points.get(acc) >= AUTOBAN_POINTS) { // See if it's sufficient to auto ban
                ...
                if (ServerConstants.getAutoban()) {
                    FileoutputUtil.logToFile("封鎖/自動封鎖.txt", "\r\n" + FileoutputUtil.CurrentReadable_TimeGMT() + "玩家: " + c.getPlayer().getName() + " 因為 " + sb + " 而被系統封鎖");
                    World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(0, "[GM密語] " + c.getPlayer().getName() + "因為使用" + sb + "而被管理員永久停權。"));
                    World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "[封鎖系統] " + c.getPlayer().getName() + "因為使用違法程式練功而被管理員永久停權。"));
                    c.getPlayer().ban(sb.toString(), false, true, false);
                    c.disconnect(true, false);
                } else {
                    FileoutputUtil.logToFile("封鎖/未自動封鎖.txt", "\r\n" + FileoutputUtil.CurrentReadable_TimeGMT() + "玩家: " + c.getPlayer().getName() + " 因為 " + sb + " 而被系統封鎖");
                }
