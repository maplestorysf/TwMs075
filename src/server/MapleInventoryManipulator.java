...
public class MapleInventoryManipulator {
    ...
    public static boolean addFromDrop(final MapleClient c, Item item, final boolean show, final boolean enhance, final boolean isPetPickup) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (isPetPickup) {
                c.sendPacket(InventoryPacket.addInventorySlot(type, item, false));
            } else {
                c.sendPacket(InventoryPacket.addInventorySlot(type, item, true));
            }
//            if (GameConstants.isHarvesting(item.getItemId())) {
...
//            }
    ...
    public static boolean removeFromSlot(final MapleClient c, final MapleInventoryType type, final short slot, short quantity, final boolean fromDrop, final boolean consume, boolean petCheck) {
        ...
        if (item != null) {
            ...
            if (petCheck && type == MapleInventoryType.CASH && c.getPlayer().getPets().size() > 0) {
//                c.getPlayer().dropMessage(5, "請卸除寵物再使用。");
                return false;
            }
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (c.getPlayer().getDebugMessage()) {
            c.getPlayer().dropMessage("穿裝備: src : " + src + " dst : " + dst + " 代碼：" + source.getItemId());
        }
...
//            chr.finishAchievement(40);
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        if (c.getPlayer().getDebugMessage()) {
            c.getPlayer().dropMessage("脫裝備: src : " + src + " dst : " + dst + " 代碼：" + source.getItemId());
        }
