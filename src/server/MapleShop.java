...
public class MapleShop {
    ...
    public void sendShop(MapleClient c) {
        if (c != null && c.getPlayer() != null) {
            ...
            c.sendPacket(NPCPacket.getNPCShop(getNpcId(), items));
        }
    }
...
//            c.getPlayer().dropMessage(1, "無法購買。");
        ...
        if (item != null && item.getPrice() > 0 && item.getReqItem() == 0) {
            ...
            if (price >= 0 && c.getPlayer().getMeso() >= price) {
                if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
                    ...
                    c.getPlayer().dropMessage(1, "道具欄滿了。");
                }
    ...
    public static MapleShop createFromDB(int id, boolean isShopId) {
        ...
        try {
            ...
            for (Integer recharge : recharges) {
                ret.addItem(new MapleShopItem((short) 1, recharge, 0, 0, 0, (byte) 0));
            }
            rs.close();
            ps.close();
