...
import constants.Types.SkillType;
...
public class MapleStatEffect implements Serializable {
    ...
    private int duration, subTime, sourceid, recipe, moveTo, t, u, v, w, x, y, z, cr, itemCon, itemConNo, fixdamage, bulletConsume, moneyCon,
        ...
        if (ret.skill) { // hack because we can't get from the datafile...
            switch (sourceid) {
                ...
                case 9001008: // hyper body
                    ...
                    break;
                case 1111002: // 鬥氣集中
                    ret.statups.put(MapleBuffStat.COMBO, 0);
                    break;
                case 5211006: // 指定攻擊
                case 5220011: // 精準砲擊
                    ret.duration = 2100000000;
...
//                case 1111003: // 黑暗之劍
//                case 1111004: // 黑暗之斧
//                    ret.monsterStatus.put(MonsterStatus.DARKNESS, 1);
//                    break;
                case 2101005: // 毒霧
                case 2111006: // 火毒合擊
                case 2121003: // 炎靈地獄
    ...
    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, int newDuration) {
        ...
        if (mpchange != 0) {
            if (mpchange < 0 && (-mpchange) > stat.getMp()) {
				applyto.getCheatTracker().registerOffense(CheatingOffense.無MP使用技能, applyto.getName() + " 在MP為" + stat.getMp() + "時使用消耗MP" + mpchange + "的技能 " + getName());
                applyto.getClient().sendPacket(CWvsContext.enableActions());
                return false;
            }
                    ...
                    if (target.getId() / 10000000 != 21 && applyto.getMapId() / 10000000 != 20) {
                        if (target.getId() / 10000000 != applyto.getMapId() / 10000000) {
                            if (target.getId() == 120000000 && applyto.getMapId() != 120000000) {
                                ...
                                return true;
                            } else if (target.getId() != 120000000 && applyto.getMapId() / 10000000 == 12) {
                                applyto.changeMap(target, target.getPortal(0));
                                return true;
                            }
                            return false;
...
//	final ScheduledFuture<?> schedule = TimerManager.getInstance().schedule(cancelAction, ((starttime + 99999) - System.currentTimeMillis()));
    ...
    private final void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final int newDuration) {
        ...
        switch (sourceid) {
            case SkillType.打手.偽裝術:
            case SkillType.暗影神偷.煙幕彈:
...
//                }
            ...
            default:
                if (isPirateMorph()) {
                    final EnumMap<MapleBuffStat, Integer> stat = new EnumMap<>(MapleBuffStat.class);
                    stat.put(MapleBuffStat.WDEF, (int) getWdef());
                    stat.put(MapleBuffStat.MDEF, (int) getMdef());
                    stat.put(MapleBuffStat.JUMP, (int) getJump());
                    stat.put(MapleBuffStat.SPEED, (int) getSpeed());
        ...
        if (hp != 0) {
            if (!skill) {
                ...
                if (!isHeal()) {
                    ...
                    hpchange += applyfrom.getStat().getMaxHp() * getHp() / 100;
                }
                if (applyfrom.hasDisease(MapleDisease.ZOMBIFY)) {
                    hpchange = -hpchange;
                }
...
//        }
    ...
    private final boolean isMist() {
        return skill && (sourceid == 2111003 || sourceid == 4221006 || sourceid == 1076 || sourceid == 11076); // poison mist, smokescreen and flame gear, recovery aura
    }
