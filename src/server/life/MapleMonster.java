    ...
    private final void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, final byte pty, final byte Class_Bonus_EXP_PERCENT, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
            ...
            if (attacker.getVip() > 0) {
                Vip_Bonus_EXP = (int) ((exp / 100.0) * attacker.getVipExpRate());
            }
//            attacker.getTrait(MapleTraitType.charisma).addExp(stats.getCharismaEXP(), attacker);
            attacker.gainExpMonster(exp, true, highestDamage, pty, Class_Bonus_EXP, Equipment_Bonus_EXP, Premium_Bonus_EXP, stats.isPartyBonus(), stats.getPartyBonusRate(), Vip_Bonus_EXP);
        }
...
//        }
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if (stats.isBoss()) {
            // 20160803
            if (stat == MonsterStatus.STUN || stat == MonsterStatus.POISON || stat == MonsterStatus.FREEZE) {
                return;
            }
        ...
        // 中毒
        if (poison && getHp() > 1) {
            int level = from.getSkillLevel(status.getSkill());
            int damage = Math.min(Short.MAX_VALUE, (int) (getMobMaxHp() / (70.0 - level) + 0.999));
            status.setX(damage);
            status.setValue(status.getStati(), damage);
            status.setPoisonSchedule(damage, from);
            if (damage > 0) {
                if (damage >= hp) {
                    damage = (int) (hp - 1);
                }
                damage(from, damage, false);
            }
        } else if (statusSkill == 3111003) {
            status.setValue(status.getStati(), (int) (getMobMaxHp() / 50.0 + 0.999));
            status.setPoisonSchedule(status.getX(), from);

            // 影網術
        } else if (statusSkill == 4111003) {
            status.setValue(status.getStati(), (int) (getMobMaxHp() / 50.0 + 0.999));
            status.setPoisonSchedule(status.getX(), from);
            // 忍影瞬殺
        ...
        final Timer.BuffTimer applyEffectTimer = Timer.BuffTimer.getInstance();
        final Runnable applyEffectTask = new Runnable() {
            ...
            public final void run() {
                if (status.getPoisonSchedule() != 0 && isAlive() && (hp - status.getPoisonSchedule()) > 1) {
                    ...
                    if (dam >= hp) {
                        dam = (int) (hp - status.getPoisonSchedule());
                    }
                    damage(from, dam, false);
                }
            }
        ...
        final Timer.BuffTimer BuffTimer = Timer.BuffTimer.getInstance();
        ...
        if (stat == MonsterStatus.POISON || stat == MonsterStatus.BURN) {
            ...
            if (con != null) {
                map.broadcastMessage(con, MobPacket.applyMonsterStatus(this, status, (int) duration), getTruePosition());
                con.getClient().sendPacket(MobPacket.applyMonsterStatus(this, status, (int) duration));
            } else {
                map.broadcastMessage(MobPacket.applyMonsterStatus(this, status, (int) duration), getTruePosition());
            }
        }
        // 20160806
        try {
