...
public class MapleQuest implements Serializable {
    ...
    public boolean canStart(MapleCharacter c, Integer npcid) {
        ...
        if (c.getQuest(this).getQuest().getId() == 1003 || c.getQuest(this).getQuest().getId() == 1004) {
            return true;
        }
    ...
    public void complete(MapleCharacter c, int npc, Integer selection) {
        if (c.getMap() != null && (autoPreComplete || checkNPCOnMap(c, npc)) && canComplete(c, npc)) {
            ...
            c.getClient().sendPacket(EffectPacket.showForeignEffect(9)); // Quest completion
            c.getMap().broadcastMessage(c, EffectPacket.showForeignEffect(c.getId(), 9), false);
        }
    }
