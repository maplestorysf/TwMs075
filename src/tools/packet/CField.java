...
//        DEFAULT_BUFFMASK |= MapleBuffStat.DEFAULT_BUFFSTAT.getValue();
    ...
    public static byte[] getServerIP(final MapleClient c, final int port, final int clientId) {
        ...
        byte[] serverIP = new byte[]{
            (byte) Integer.parseInt(ServerConstants.Gateway_IP[0]),
            (byte) Integer.parseInt(ServerConstants.Gateway_IP[1]),
            (byte) Integer.parseInt(ServerConstants.Gateway_IP[2]),
            (byte) Integer.parseInt(ServerConstants.Gateway_IP[3])};

        mplew.writeShort(0);
        mplew.write(serverIP);
...
//            }
            ...
            if (direction != (byte) 3 || skillid == 1320006) {
    ...
    public static class SummonPacket {
        ...
        public static byte[] spawnSummon(MapleSummon summon, boolean animated) {
            ...
            mplew.write(1/*summon.getSummonType()*/); // 0 = Summon can't attack - but puppets don't attack with 1 either ^.-
...
//            mplew.writeZeroBytes(10);
    ...
    public static class NPCPacket {
        ...
        public static byte[] getNPCShop(int sid, List<MapleShopItem> items) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] boatEffect(int effect, int mode) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//            mplew.writeInt(pDMG);
    ...
    public static byte[] spawnMist(final MapleMist mist) {
        ...
        mplew.write(0); //2 = invincible, so put 1 for recovery aura
        mplew.writeInt(mist.getOwnerId());
        mplew.writeInt(mist.getMobSkill() == null ? mist.getSourceSkill().getId() : mist.getMobSkill().getSkillId());
        mplew.write(mist.getSkillLevel());
        mplew.writeShort(mist.getSkillDelay());
        mplew.writeRect(mist.getBox());
        mplew.write(mist.isShelter());
