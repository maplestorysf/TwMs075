...
//        }
    ...
    public static byte[] spawnMonster(MapleMonster life, int spawnType, int link) {
        ...
        mplew.write(HexTool.getByteArrayFromHexString("00 00 00 08 00 00 00 00"));
    ...
    public static byte[] applyMonsterStatus(final MapleMonster mons, final MonsterStatusEffect ms, int time) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.APPLY_MONSTER_STATUS.getValue());
        mplew.writeInt(mons.getObjectId());
        //PacketHelper.writeSingleMask(mplew, ms.getStati());
        mplew.writeInt(ms.getStati().getValue());
...
//        }
        ...
        mplew.writeShort(time / 100); // delay in ms
        mplew.write(1); // size
    ...
    public static byte[] applyMonsterStatus(final MapleMonster mons, final List<MonsterStatusEffect> mse) {
        ...
        if (ms.getStati() == MonsterStatus.POISON) { // burn also here
            ...
            mplew.writeInt(MonsterStatus.POISON.getValue());
